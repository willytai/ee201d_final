...
def f2b(design_info_bot, design_info_top, tech_const, design_netlist_bot, design_netlist_top, script_dir):
...
'''.format(coreDim,
    ...
    with open(os.path.join(script_dir, 'timing_check_f2b_bot.tcl'), 'w') as f:
        f.write(timingCheckBot)
    with open(os.path.join(script_dir, 'timing_check_f2b_top.tcl'), 'w') as f:
        f.write(timingCheckTop)

    # return finalArea, defectDensity, tsvCount, wireBonds
    return finalArea, constraints['defectDens'], tsvCount, botInfo['ioCount'], botInfo['designPeriod']
...
def main(args):
    return f2b(args.design_info_bot, args.design_info_top, args.tech_const, args.design_netlist_bot, args.design_netlist_top, args.script_dir)
